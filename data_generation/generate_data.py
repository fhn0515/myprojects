...
def generate_cinemas(count=50):
    ...
    cinemas = []
    province_city = {
        '北京市': ['北京市'], '天津市': ['天津市'], '河北省': ['石家庄市', '唐山市', '保定市', '邯郸市', '秦皇岛市'],
        '山西省': ['太原市', '大同市', '晋中市', '临汾市', '长治市'], '内蒙古自治区': ['呼和浩特市', '包头市', '鄂尔多斯市', '赤峰市'],
        '上海市': ['上海市'], '江苏省': ['南京市', '苏州市', '无锡市', '常州市', '徐州市'],
        '浙江省': ['杭州市', '宁波市', '温州市', '嘉兴市', '绍兴市'], '安徽省': ['合肥市', '芜湖市', '蚌埠市', '安庆市', '马鞍山市'],
        '福建省': ['福州市', '厦门市', '泉州市', '漳州市', '莆田市'], '江西省': ['南昌市', '赣州市', '九江市', '上饶市', '宜春市'],
        '山东省': ['济南市', '青岛市', '烟台市', '潍坊市', '临沂市'],
        '广东省': ['广州市', '深圳市', '东莞市', '佛山市', '中山市'], '广西壮族自治区': ['南宁市', '柳州市', '桂林市', '玉林市', '北海市'],
        '海南省': ['海口市', '三亚市', '儋州市', '文昌市'],
        '重庆市': ['重庆市'], '四川省': ['成都市', '绵阳市', '德阳市', '宜宾市', '南充市'],
        '贵州省': ['贵阳市', '遵义市', '六盘水市', '毕节市'], '云南省': ['昆明市', '大理市', '丽江市', '曲靖市', '玉溪市'],
        '西藏自治区': ['拉萨市', '日喀则市', '昌都市'],
        '河南省': ['郑州市', '洛阳市', '开封市', '新乡市', '南阳市'], '湖北省': ['武汉市', '宜昌市', '襄阳市', '荆州市', '十堰市'],
        '湖南省': ['长沙市', '株洲市', '湘潭市', '衡阳市', '岳阳市'],
        '陕西省': ['西安市', '宝鸡市', '咸阳市', '榆林市', '汉中市'], '甘肃省': ['兰州市', '天水市', '酒泉市', '张掖市'],
        '青海省': ['西宁市', '海东市'], '宁夏回族自治区': ['银川市', '石嘴山市', '吴忠市'],
        '新疆维吾尔自治区': ['乌鲁木齐市', '喀什市', '克拉玛依市'],
        '辽宁省': ['沈阳市', '大连市', '鞍山市', '锦州市', '抚顺市'], '吉林省': ['长春市', '吉林市', '四平市'],
    ...
    for i in range(1, count + 1):
        ...
        cinema_name = f"{random.choice(cinema_prefix)}{random.randint(1, 5)}店"
...
def generate_box_office(movies, cinemas, total_records=500000):
    """生成票房数据"""
    box_office_list = []
    generated_keys = set()
    records_per_pair = max(1, total_records // (len(movies) * len(cinemas) + 1))
    ...
    for cinema in cinemas:
        ...
        for movie in selected_movies:
            ...
            day_count = min(records_per_pair, random.randint(1, 10))
            for _ in range(day_count):
                ...
                if len(box_office_list) >= total_records:
...
def generate_users(count=10000):
    ...
    province_city = {
        ...
        '江苏省': ['南京市', '苏州市'], '浙江省': ['杭州市', '宁波市'], '四川省': ['成都市'],
        '湖北省': ['武汉市'], '陕西省': ['西安市'], '河南省': ['郑州市'], '山东省': ['青岛市', '济南市']
    }
...
def generate_comments(movies, users, total_records=100000):
    """生成评论数据"""
    comments = []
    comment_templates = [
    ...
    records_per_movie = max(1, total_records // len(selected_movies))

    for movie in selected_movies:
        select_count = min(records_per_movie, len(users))
...
def main_importer():
    ...
    try:
        ...
        if bulk_insert(connection, 'movie_info', movies) == -1:
            raise Exception("movie_info 插入失败")
        if bulk_insert(connection, 'cinema_info', cinemas) == -1:
            raise Exception("cinema_info 插入失败")
        if bulk_insert(connection, 'user_info', users) == -1:
            raise Exception("user_info 插入失败")
        if bulk_insert(connection, 'box_office', box_office) == -1:
            raise Exception("box_office 插入失败")
        if bulk_insert(connection, 'comment', comments) == -1:
            raise Exception("comment 插入失败")
